...
RSpec.describe 'facilities', type: :request do
  ...
  describe '/v0/vaos/facilities/:id/clinics' do
    context 'with a loa1 user' do
      let(:user) { FactoryBot.create(:user, :loa1) }
  ...
  describe '/v0/vaos/facilities/:id/visits/:schedule_type' do
    context 'with a loa1 user' do
      let(:user) { FactoryBot.create(:user, :loa1) }

      it 'returns a forbidden error' do
        get '/v0/vaos/facilities/984/visits/direct', params: { type_of_care_id: '323', system_id: '984GA' }
        expect(response).to have_http_status(:forbidden)
        expect(JSON.parse(response.body)['errors'].first['detail'])
          .to eq('You do not have access to online scheduling')
      end
      ...
      it 'returns a forbidden error' do
        get '/v0/vaos/facilities/984/visits/request', params: { type_of_care_id: '323', system_id: '984GA' }
        expect(response).to have_http_status(:forbidden)
        expect(JSON.parse(response.body)['errors'].first['detail'])
          .to eq('You do not have access to online scheduling')
      end
